...
export async function renderPanelImage({ prompt, styleBible, lessonId, panelCode, characterActions }) {
  ...
  // On Vercel the filesystem is read-only, so upload to Vercel Blob when a token
  // is configured. Locally (no token) write into public/ for a /assets URL.
  if (process.env.BLOB_READ_WRITE_TOKEN) {
    const { put } = await import("@vercel/blob");
    const blob = await put(`generated/${lessonId}/${panelCode}.png`, buffer, {
      access: "public",
